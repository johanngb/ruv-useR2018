...
author: "Ramyar Molania and Anna Quaglieri"
output:
  html_document:
    df_print: paged
    toc: yes
...
knitr::opts_chunk$set(echo = TRUE,eval=TRUE,cache=TRUE,tidy = TRUE)
library(here)
```
...
ruv_svdplot(t(ExpressionData), k = c(1,2),info = SampleInformation) + 
...
```{r}
ruv_svdplot(t(ExpressionData), k = c(1,3),info = SampleInformation) + 
  geom_point(aes(color=Batch,shape=Tissue)) +
  scale_color_manual(values=ColorBatch) +
  labs(color="Batches")
```
...
ruv_rle(Y = t(ExpressionData),ylim = c(-3 , 3),rowinfo = SampleInformation) + 
  ...
  labs(colour="Batches") +
  scale_color_manual(values=ColorBatch) + 
  geom_hline(yintercept = 0,linetype="dotted",colour="cyan") +
  ggtitle('Unnormalized data')
```


```{r}
ruv_rle(Y = t(ExpressionData),ylim = c(-3 , 3),rowinfo = SampleInformation) + 
  geom_point(aes(x = rle.x.factor,y= middle,colour=Tissue)) + 
  theme(legend.position = "bottom") + labs(colour="Tissues")  + 
  geom_hline(yintercept = 0,linetype="dotted",colour="cyan") + 
  ggtitle('Unnormalized data')
```
...
```{r}
RUVcorrected <- RUVIII(Y = t(ExpressionData), 
                       M = ReplicateMatrix, 
                       ctl = c(1:nrow(ExpressionData)), 
                       k = 46)
```
...
```{r}
ruv_svdplot(RUVcorrected, k = c(1,2),info=SampleInformation) + 
  geom_point(aes(color=Tissue)) + 
  labs(color="Tissues")
```
...
```{r}
ColorBatch <- c("#c51b8a", "blue", "darkgreen") 
ruv_rle(Y = RUVcorrected,ylim = c(-3 , 3)) + 
  geom_point(aes(x = rle.x.factor,y= middle,colour=SampleInformation$Batch)) + 
  theme(legend.position = "bottom") + 
  labs(colour="Batches") +
  scale_color_manual(values=ColorBatch) + 
  geom_hline(yintercept = 0,linetype="dotted",colour="cyan")+
  ggtitle('Normalized data with RUV-III')
```
